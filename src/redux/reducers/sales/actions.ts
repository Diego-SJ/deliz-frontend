...
const customActions = {
      ...
      if (!salesList.length || args?.refetch) {
        ...
        const supabaseQuery = supabase.rpc('fetch_sales', {
          ...
          status_ids_param: !filters?.status_id
            ? [
                STATUS_DATA.PAID.id,
                STATUS_DATA.PENDING.id,
                STATUS_DATA.CANCELED.id,
              ]
            : [filters?.status_id],
          branch_id_param:
            !isAdmin || (!!branch_id && branch_id !== 'ALL')
              ? branch_id || currentBranch?.branch_id
              : null,
          cash_register_ids_param: !isAdmin
  ...
  getSaleById:
    (args?: { sale_id?: number } & FetchFunction) =>
    async (dispatch: AppDispatch, getState: AppState) => {
      try {
          ...
          let saleDetails: SaleDetails | undefined =
            metadata && metadata[0]
              ? ({
                  ...metadata[0],
                  amount_paid:
                    metadata[0]?.status_id === STATUS_DATA.PAID.id
                      ? metadata[0]?.amount_paid
                      : 0,
                } as SaleDetails)
              : undefined;

          dispatch(
            salesActions.setCurrentSale({ items, metadata: saleDetails }),
          );
          dispatch(salesActions.setLoading(false));
  ...
  addItemToSale:
    (item: SaleItem) => async (dispatch: AppDispatch, getState: AppState) => {
      ...
      await dispatch(
        salesActions.getSaleById({ refetch: true, sale_id: item.sale_id }),
      );
    },
  createSale:
    (sale: Partial<Sale>) =>
      ...
      try {
        ...
        const { data, error } = await supabase
          .rpc('create_sale', {
            ...
            p_customer_id: state?.sales?.cash_register.customer_id as number,
            p_discount: state?.sales?.cash_register.discount,
            p_discount_type:
              state?.sales?.cash_register.discountType || 'AMOUNT',
            p_payment_method: sale?.payment_method || 'EFECTIVO',
            p_shipping: state?.sales?.cash_register.shipping,
            p_status_id: sale.status_id || STATUS_DATA.PENDING.id,
            p_total: sale.total || 0,
            p_created_by: state?.users?.user_auth?.profile?.profile_id,
          })
          .select()
          .single();
        dispatch(salesActions.setLoading(false));
  ...
  updateSale:
    (item: Sale) => async (dispatch: AppDispatch, getState: AppState) => {
      try {
  ...
  updateSaleItem: (item: SaleItem) => async (dispatch: AppDispatch) => {
    try {
      ...
      await dispatch(
        salesActions.getSaleById({ refetch: true, sale_id: newItem.sale_id }),
      );
  ...
  deleteItemById:
    (id: number) => async (dispatch: AppDispatch, getState: AppState) => {
      try {
        const sale = getState().sales.current_sale;
  ...
  cashRegister: {
    ...
    add:
      (newItem: Partial<CashRegisterItem>) =>
      async (dispatch: AppDispatch, getState: AppState) => {
        const { customer_id = null, price_id = null } =
          getState().sales.cash_register;
        const price = productHelpers.getProductPrice(
          newItem.product as Product,
          price_id,
        );
        ...
        const itemDefaultExists = currentItems.findIndex((item) => {
          return (
            ...
            item.price_type === 'DEFAULT'
          );
        });

        if (itemDefaultExists >= 0) {
          newItemData.quantity =
            currentItems[itemDefaultExists].quantity + newItemData.quantity;
    ...
    changePrice:
      ...
      async (dispatch: AppDispatch, getState: AppState) => {
        ...
        items = items.map((item) => {
          let price = productHelpers.getProductPrice(
        ...
        dispatch(
          salesActions.updateCashRegister({ items, price_id: price_id }),
        );
      },
    ...
    applyDiscount:
      ...
      async (dispatch: AppDispatch, getState: AppState) => {
        let items = [...(getState().sales.cash_register?.items ?? [])]?.filter(
          Boolean,
        );
    ...
    setCustomerId:
      (customer_id: number | null) => async (dispatch: AppDispatch) => {
        dispatch(salesActions.updateCashRegister({ customer_id }));
      },
  },
  closeDay:
    (comment?: string) => async (dispatch: AppDispatch, getState: AppState) => {
      ...
      dispatch(
        salesActions.setClosingDays({ data: newData, today_is_done: true }),
      );
