        ...
        <thead className="bg-white !sticky top-[60px] z-10">
          <tr>
            {TABLE_HEADER.map((col, index) => {
              if (index === 0) {
                return (
                  <th key={col.id} className="w-auto  min-w-[200px] px-6 pt-2 pb-2 text-left text-sm font-normal" scope="col">
              ...
              return (
                <th key={col.id} className="w-1/5 px-0 text-left text-sm font-normal" scope="col">
                  <span className="flex flex-col px-6 pr-2 pt-2 gap-1.5">
                    <span className="flex flex-col xl:flex-row xl:items-end gap-1">
                      ...
                      <p className="text-gray-400 -my-1 xl:m-0">
                        <span className="text-lg font-light mr-1 tracking-tighter">${col.price?.monthly}</span>
                ...
                {row.cols.map((col, index) => {
                  if (index === 0) {
                    return (
                      <th
                        key={row.id + index}
                        className="flex items-center px-6 py-5 last:pb-24 text-left text-xs font-normal "
                        scope="row"
                      >
                        <span>
                          {col.title}{' '}
                          {!!(col as any)?.tooltip && (
                            <Tooltip title={(col as any)?.tooltip} placement="top">
                  ...
                  return (
                    <td key={row.id + index} className="pl-6 pr-2 tier-pro ">
                      <div className="text-xs flex items-center gap-2">
                        <span className="flex items-center">{col?.title}</span>
                        {!!(col as any)?.tooltip && (
                          <Tooltip title={(col as any)?.tooltip} placement="top">
                            <InfoCircleOutlined className="text-gray-800 cursor-help" />
                          </Tooltip>
