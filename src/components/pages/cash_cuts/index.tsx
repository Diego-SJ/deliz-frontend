...
export const getStatus = (total = 0, received = 0) => {
  if (total === received)
    return { color: 'green', text: 'Cerrada sin diferencias' };
  if (total > received)
    return {
      color: 'red',
      text: `Faltante de ${functions.money(total - received)}`,
    };
  return {
    color: 'blue',
    text: `Sobrante de ${functions.money(received - total)}`,
  };
};
...
const columns: ColumnsType<CashCut> = [
  {
    ...
    width: 220,
    render: (value = '') => (
      <span className="pl-3">{functions.tableDate(value)}</span>
    ),
  },
  {
...
const CloseSales = () => {
  ...
  const { cash_registers, branches } = useAppSelector(
    ({ branches }) => branches,
  );
  ...
  const onRowClick = (record: CashCut) => {
    navigation(
      APP_ROUTES.PRIVATE.TRANSACTIONS.CASHIER_DETAIL
        .hash`${record!.cash_cut_id!}`,
    );
  };
          ...
          <Row gutter={[10, 10]} className="!mb-4">
              ...
              <Dropdown
                menu={{
                  ...
                  onClick: ({ key }) => {
                    ...
                    dispatch(
                      cashiersActions.cash_cuts.fetchCashCutsByCompany(),
                    );
                  },
                }}
              >
                <Button
                  ...
                  icon={<Building className="text-black !w-4 !h-4" />}
                >
                  Sucursal
                </Button>
              </Dropdown>
            ...
            <Col lg={4} md={12} xs={12}>
              <Dropdown
                menu={{
                  defaultSelectedKeys: [
                    cash_cuts?.filters?.cash_register_id || '',
                  ],
              ...
              >
                <Button
                  block
                  size={isTablet ? 'large' : 'middle'}
                  icon={<CashRegisterSvg className="text-black !w-4 !h-4" />}
                >
                  Caja
                </Button>
              </Dropdown>
              ...
              <Dropdown
                menu={{
                  ...
                  items: [
                    ...
                    {
                      key: 'closing_date,true',
                      label: 'Fecha de cierre (antiguos primero)',
                    },
                    {
                      key: 'opening_date,false',
                      label: 'Fecha de apertura (recientes primero)',
                    },
                    {
                      key: 'opening_date,true',
                      label: 'Fecha de apertura (antiguos primero)',
                    },
                    {
                      key: 'initial_amount,false',
                      label: 'Monto de apertura (mayor primero)',
                    },
                    {
                      key: 'initial_amount,true',
                      label: 'Monto de apertura (menor primero)',
                    },
                    {
                      key: 'final_amount,false',
                      label: 'Monto esperado (mayor primero)',
                    },
                    {
                      key: 'final_amount,true',
                      label: 'Monto esperado (menor primero)',
                    },
                    {
                      key: 'received_amount,false',
                      label: 'Monto recibido (mayor primero)',
                    },
                    {
                      key: 'received_amount,true',
                      label: 'Monto recibido (menor primero)',
                    },
                  ],
                  selectable: true,
                  onClick: ({ key }) => {
                    ...
                    <div className="flex w-full justify-between mb-1">
                      <div className="flex flex-col">
                        <Typography.Paragraph className="!mb-0 flex gap-2 items-center text-gray-800">
                          <LockKeyholeOpen className="w-4" />{' '}
                          <span>{functions.tableDate(item?.opening_date)}</span>
                        </Typography.Paragraph>

                        <Typography.Paragraph className="flex gap-2 items-center text-gray-800">
                          <LockKeyhole className="w-4" />{' '}
                          <span>{functions.tableDate(item?.closing_date)}</span>
                        </Typography.Paragraph>
                      </div>
                    ...
                    <div className="grid w-full grid-cols-3">
                      <div className="!text-xs flex-col-reverse text-center text-gray-400 inline-flex">
                        ...
                        <span className="font-medium block">
                          {functions.money(item.initial_amount)}
                        </span>
                      </div>
                      <div className="!text-xs flex-col-reverse text-center text-gray-400 font-light inline-flex">
                        ...
                        <span className="font-medium block">
                          {functions.money(item.final_amount)}
                        </span>
                      </div>
                      <div className="!text-xs flex-col-reverse text-center text-gray-400 font-light inline-flex">
                        <span className="font-light mr-1">Recibido</span>{' '}
                        <span className="font-medium block">
                          {functions.money(item.received_amount)}
                        </span>
                      </div>
                    </div>
                  </div>
